...
void RotateWidget::initRotateWindow()
{
    ...
    //this->setGeometry(contentsRect());
    //this->move((QApplication::desktop()->width() - this->width()) / 2, (QApplication::desktop()->height() - this->height()) / 2);//窗口居中显示
}

// 开始旋转窗口;
void RotateWidget::onRotateWindow()
{
    //qDebug()<<"--------------- RotateWidget::onRotateWindow() ------------------";
    // 如果窗口正在旋转，直接返回;
